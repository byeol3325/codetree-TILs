...
def Collapse(R, santa, opt):
    ...
    if opt == 'r': #루돌프가 와서 충돌
        ...
        santa.r -= santa.power*dr[santa.d]; santa.c -= santa.power*dc[santa.d]
        santa.d = (santa.d+2)%4; santa.stun = 2
        if santa.r <= 0 or santa.c <= 0 or santa.r >= N+1 or santa.c >= N+1: #밖임
            santa.out = 1 # 밖이라 out
        elif board[santa.r][santa.c] != 0: # 다른 산타가 있음
...
for m in range(1, M+1):
    ...
    if m == stop:
        ...
        if show_santas == 1:
            for k, v in santas.items():
                print(k, "santa info (idx, r,c, stun, out, score, d) : ", v.getInfo())
        break
