...
def Find_camp(num): #num에 time으로 들어올거임
    ...
    base_camps.pop(min_dis_loc[0][2]) # 가장 가까운 basecamp로 찜. 해당 basecamp는 제거
...
while True:
    ...
    if time_ <= M: # m초전까지는 사람들 계속 들어옴
        # 어느 베이스캠프에서 시작할지 정하기
        person = Find_camp(time_) # 어느 베이스캠프갈지 찾음
        matrix[person[0]][person[1]] = 2 # 어디서 시작할지 정하고 거기 이제 못감 ㅅㄱ
        people.append(person) # 사람 추가됨
