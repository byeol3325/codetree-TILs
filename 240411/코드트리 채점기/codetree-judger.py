...
waiting_queue = [] # 대기 큐.
waiting_queue.append([1,0,u0]) # 우선순위(p), 시간(t), [url, id] 어차피 오름차순 시간으로 들어옴.
...
def Rule3(q): # 300 t. 채점해도되나 안되나 판별
    ...
    cal_idx = -1 # 일단 제일 먼저 비어있는 계산기. 일단 기록. ## 비어있으면
    doing_domains = []
    for i in range(1, N+1):
        if calculaters[i] != []:
            domain = calculaters[i][2][0]
            doing_domains.append(domain)
        if calculaters[i] == []:
            if cal_idx == -1:
                cal_idx = i
    
    if cal_idx == -1: # 모든 계산기가 돌아가고 있음
        return
...
def Rule4(q): # 400 t J_id
    ...
    end_task[domain] = [start_time, end] # 시작시간, 끝나는시간 기록
    calculaters[J_id] = [] # 다 끝났으니 계산기 비우기.
    return
